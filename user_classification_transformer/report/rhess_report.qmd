...
Also, some twitter accounts might be fictious [@ahmed_using_2017].
Especially the detection of bots has triggered a lot of research [e.g. @kudugunta_deep_2018].
When researchers analyze Twitter data, they may want to exclude these cases from their datasets.
Or they might also want to isolate users from a certain country [@kwon_disentangling_2018] or gender [@vashisth_gender_2020], depending on the research questions.
...
As institutional accounts make up 9% of Twitter accounts [@mccorriston_organizations_2015] they can skew the tweet sample significantly.
...
@yan_classifying_2013 differentiate between *open* and *closed* accounts, where open accounts publish information to the public, with their goal being to promote products, services or themselves.
...
Some differences between institutional and general public users have been highlighted by @kwon_disentangling_2018.
...
This can improve the prediction performance on different NLP tasks [@wankmuller_introduction_2022].
...
It has been trained on 850 million english tweets and consists of 135 million parameters [@nguyen_bertweet_2020].
...
The only dataset we could find was that used by @kwon_disentangling_2018, who used Twitter profile descriptions as the sole input for their classifier.
...
However, for the other sources, the distribution of positive and negative cases matched the description of the authors making them seem reliable enough for us to analyze.
...
However, as the original study's authors used the scikit learn library for training []@kwon_disentangling_2018], it is likely that they shuffled the data as well before training.
...
![Distribution over token lengths in the full dataset](figs/token_lengths.png){#fig-token}
...
The hyperparameters used and the corresponding search spaces are shown in @tbl-search_spaces.
...
For comparison, the results of the original study are shown in @tbl-results_original. 
...
: The original study's results using a random forrest classifier [@kwon_disentangling_2018] {#tbl-results_original}

To evaluate whether the results support our hypotheses, we have to look at four different performance measures. @kwon_disentangling_2018 measured Accuracy, Precision, Recall, and F1 to evaluate the performance of their model. 

To simplify our hypotheses, we expected improvements in all four measures by using transformers and transfer learning. 

As to _H1.A_, our expectation did not hold. Only Recall is improved compared to the original study, the other measures show lower values. 

For _H1.B_, our expectation of a consistently improved generalization performance also did not hold. However, that is only due to a slightly worse precision when training on the boston data and testing on the brussels dataset. All other measures were improved compared to the original study. 
